...
@CucumberOptions(
        features = "src/test/resources/features/publisher/invalid_token_invocation.feature",
        glue = "org.wso2.am.integration.cucumbertests.stepdefinitions",
        plugin = {"pretty", "html:target/cucumber-report/invalid-token-invocation.html"}
)
@Test(dependsOnGroups = {"tenantInitialization"},
    groups = {Constants.TEST_GROUPS.CORE, Constants.TEST_GROUPS.EXTENDED, Constants.TEST_DOMAINS.PUBLISHER})
public class InvalidTokenInvocationRunner extends AbstractTestNGCucumberTests {
    ...
    @BeforeClass(alwaysRun = true, dependsOnGroups = {"tenantInitialization"})
    ...
    @AfterClass(alwaysRun = true, dependsOnGroups = {"tenantInitialization"})
    ...
    public static Object[] factory(String tenantDomain, String userKey) {
        InvalidTokenInvocationRunner runner = new InvalidTokenInvocationRunner();
    ...
    @Override
    @Test(dataProvider = "scenarios", dependsOnGroups = {"tenantInitialization"},
            groups = {Constants.TEST_GROUPS.CORE, Constants.TEST_GROUPS.EXTENDED, Constants.TEST_DOMAINS.PUBLISHER})
    ...
    public Object[][] userModeDataProvider() {
        return new Object[][]{
                {Constants.SUPER_TENANT_DOMAIN, Constants.ADMIN_USER_KEY},
                {Constants.NEW_TENANT_DOMAIN, Constants.NEW_TENANT_ADMIN_USER_KEY}
        };
    }
}
