...
Feature: Publisher API Definition Import
  ...
  # round-trip of APIImportExportTestCase. Uses a binary download (doGetToFile) so the zip is not corrupted.
  # Runs as admin: import (POST /apis/import) needs the apim:api_import_export scope the least-privilege publisher
  # role lacks (export/GET works for it, but import/POST returns 401).
