...
Feature: Key Manager Token Issuance
  ...
  # TokenEncryptionScope (strict scope-in-token): a shared scope bound to the admin role, attached to a subscribed
  # API, is GRANTED in the issued token — the password-grant token response's scope field carries it. Ports
  # TokenEncryptionScopeTestCase (the scope-in-token behaviour; token encryption is a transparent at-rest config
  # that does not change scope issuance, so no encryption overlay is needed).
