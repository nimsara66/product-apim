...
Feature: Publisher API Lifecycle
  ...
  Scenario Outline: A subscriber-role user cannot create an API as <actor>
    ...
    # role enforcement in both tenants. This is also the observable of APICreationForTenantsTestCase (a role
    # without the api/create permission cannot create an API); the legacy's dynamic create-role-then-remove-
    # permission mechanism is not reproduced (it would need a UserAdmin SOAP role-permission harness), but the
    # security gate it verifies — no api_create permission => create denied — is covered here.
    Given The system is ready and I have valid publisher access tokens as "<actor>"
  ...
  # confirming name uniqueness is case-insensitive.
