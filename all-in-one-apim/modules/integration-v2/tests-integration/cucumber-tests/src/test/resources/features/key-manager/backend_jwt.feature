...
Feature: Backend JWT Claims
  ...
  app/API/subscriber claims). User-profile claims (givenname/lastname/mobile/organization) need claim-mapping
  setup and are tracked separately.
  ...
  # token whose subject is the dotted user (the repeat call failed before the fix). Super-tenant only — the dot,
  # not the tenant, is the concern. Ports JWTDecodingTestCase.
  @cap:key-manager @feat:backend-jwt @rule:dotted-username @type:regression @dep:gateway @legacy:JWTDecodingTestCase
  Scenario: The backend JWT is generated for a dotted-username ([string].[string]) resource owner
    Given The system is ready
    And I have valid access tokens as "admin"
    And I provision user "jwtdecode.user" with roles "Internal/subscriber"
    ...
    #  — a clean 200 is the proof that backend-JWT generation succeeded for the dotted-username owner, matching
    #  the legacy which asserts only the status code.)
  ...
  # password-grant token is minted for that user so the gateway generates the backend JWT with their profile.
  # Super-tenant only. Ports the user-profile-claims part of JWTTestCase.
  @cap:key-manager @feat:backend-jwt @rule:user-profile-claims @type:regression @dep:gateway @legacy:JWTTestCase
  Scenario: The backend JWT carries the resource owner's profile claims
    Given The system is ready
    And I have valid access tokens as "admin"
    And I register the OIDC user-profile claim mappings and scope
    And I provision user "jwtclaims.user" with roles "Internal/subscriber"
    And I set the user claim "http://wso2.org/claims/givenname" to "ProfileFirstName" for user "jwtclaims.user"
    And I set the user claim "http://wso2.org/claims/lastname" to "ProfileLastName" for user "jwtclaims.user"
    And I set the user claim "http://wso2.org/claims/mobile" to "94123456987" for user "jwtclaims.user"
    And I set the user claim "http://wso2.org/claims/organization" to "ProfileOrgABC" for user "jwtclaims.user"
    ...
    # Configure the app's OAuth service provider to REQUEST the user-profile claims (the backend JWT only
    # surfaces claims the SP requests). Must happen before the token is minted.
    When I configure the service provider for consumer key "consumerKey" to request the user-profile claims

    # Mint a password-grant token (openid scope, so the KM associates the OIDC/user claims with the token) whose
    # resource owner is the claim-populated user, then invoke.
    When I act as "jwtclaims.user"
    ...
    Then The response status code should be 200
    # The backend JWT carries the user's profile claims.
