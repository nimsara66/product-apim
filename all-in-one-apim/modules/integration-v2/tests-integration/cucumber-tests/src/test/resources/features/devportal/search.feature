...
Feature: DevPortal Search & Discovery
  ...
  # Store search by TAG: a common tag shared by two APIs returns both; a distinct tag returns only its API.
  # Tags are uniquely generated (a shared common tag stored once + a distinct tag each) so parallel scenarios
  # never collide in the shared Solr index. Ports APISearchAPIByTagTestCase (search-by-tag).
  @cap:devportal @feat:discovery @rule:tag-search @type:regression @legacy:APISearchAPIByTagTestCase
  ...
  # the returned page count saturates at the limit. Super-tenant only (a tenant-agnostic count test; kept ×1
  # since it is heavy — 12 APIs). Ports SearchPaginatedAPIsWithMultipleStatusTestCase.
  @cap:devportal @feat:discovery @rule:pagination @type:regression @legacy:SearchPaginatedAPIsWithMultipleStatusTestCase
  Scenario: DevPortal paginated search caps the page at the requested limit
  ...
  @cap:devportal @feat:discovery @rule:tag-cloud @type:regression @legacy:APISearchAPIByTagTestCase
  ...
  @cap:devportal @feat:discovery @rule:latest-version @type:regression @legacy:APIMANAGER4081PaginationCountTestCase
