...
Feature: Gateway CORS
  ...
  @cap:gateway @feat:cors @type:regression @dep:publisher @legacy:CORSAccessControlAllowCredentialsHeaderTestCase
  Scenario Outline: CORS allow-origin and allow-credentials headers are returned for a matching origin as <actor>
    ...
    Then The response status code should be 200
    And The response header "Access-Control-Allow-Origin" should be "http://localhost"
