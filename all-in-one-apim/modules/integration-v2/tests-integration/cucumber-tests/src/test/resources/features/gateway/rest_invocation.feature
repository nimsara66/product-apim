...
Feature: Gateway REST API Invocation
  ...
  @cap:gateway @feat:rest-invocation @rule:uri-encoding @type:regression @dep:publisher @legacy:APIResourceWithTemplateTestCase @legacy:UriTemplateReservedCharacterEncodingTest
  Scenario Outline: An encoded URI path segment is routed through a templated endpoint to the backend as <actor>
    ...
    # Reserved char (a colon) in a QUERY parameter value: sent RAW; the gateway forwards it to the backend
    # VERBATIM (APIM:WSO2 — verify-first: 4.7.0 does NOT percent-encode it; the legacy's expected %3A came from
    # its special raw-Synapse uri-template-encoding.xml config, not default gateway behaviour). This pins the
    # black-box-observable part of UriTemplateReservedCharacterEncodingTest (the reserved char reaches the backend
    # intact); its escapeUrlEncoded + message-context-property-decode variants are Synapse-internal and not
    # black-box observable, so they are intentionally out of scope.
    When I invoke the API at raw gateway context "{{uriApiContext}}/1.0.0/qseg?queryParam=APIM:WSO2" using access token "generatedAccessToken" until response status code becomes 200 within 60 seconds
    Then The response status code should be 200
    And The response should contain "queryParam=APIM:WSO2"
  ...
  # so the response body reveals which backend was hit. Super-tenant only (heavy: dual key/token generation),
  # tenant-agnostic. Ports the routing case of InvokeAPIWithVariousEndpointsAndTokensTestCase.
  ...
  Scenario: A production token routes to the production endpoint and a sandbox token to the sandbox endpoint
    Given The system is ready
  ...
  Scenario: A sandbox token to a production-only API is rejected with 900901
    Given The system is ready
    And I have valid access tokens as "admin"
  ...
  @cap:gateway @feat:rest-invocation @rule:endpoint-routing @type:negative @dep:publisher @legacy:InvokeAPIWithVariousEndpointsAndTokensTestCase
  Scenario: A production token to a sandbox-only API is rejected with 900901
    Given The system is ready
    And I have valid access tokens as "admin"
