...
| group2 | — | not started |
| group3 | — | not started |
| group4 | [group4.md](group4.md) | ✅ **COMPLETE — every cleanly-portable class done (~31 ported & verified ×2 tenant)**, on branch `test-framework-v2_backend-port-G4`. Built 5 reusable harnesses: API export/import (binary download), node-soap WSDL service (nodebackend:3021), sequence-backend upload, Location backend routes, operation-policy attach. Highlights: search (tag/pagination/tag-cloud/latest-version), lifecycle (case-dup 409, block/deprecate, demote-retains-sub, custom-LC), gateway (PATCH/HEAD, prod-sandbox routing + 900901, CORS, mediation op-policies incl. **export/import + secret attributes**, JWT-claim validator, sequence-backend, Location, custom api-key header), backend-JWT (default claims + **dotted-username**), URL-safe JWT, scope-in-token, API-categories, endpoint-validation, resource-order, WSDL-import, SOAP-to-REST, SOAP + REST export/import, throttle-reset. Attributed/covered (dedup): SharedScope, InvalidToken, DefaultVersion, SandboxEnv, APICreationForTenants, **JWTRequestCountThrottling (conditional-group CRUD + real 429 enforcement covered by admin/throttling_policy + gateway/throttling_enforcement)**. Also **ESBJAVA3380** (colon-key JSON→XML via the shipped jsonToXML policy + /reflect-body), **ScriptMediator** (JS `<script>` runs on the bundled Rhino engine — handles a null object field; the "JDK21 Nashorn removed" deferral was WRONG), and **UriTemplate** query-param reserved-char passthrough (verify-first: 4.7.0 forwards it verbatim, not %3A). Also **JWT user-profile claims** (givenname/lastname/mobile/organization in the backend JWT — built the full SOAP-admin harness set: setUserClaimValue + addExternalClaim + updateScope + the IdentityApplicationManagementService SP requested-claims round-trip + `enable_user_claims`/`use_username_as_sub_claim` toml + openid-profile scope). **Deferred tail — 1, root cause in `group4-port-backlog.md`:** JWTRequestCountThrottling conditional-group *runtime* enforcement — **verified 4.7.0 PRODUCT GAP** (rigorously re-tested ×4: high-default isolation, routing confirmed, condition Siddhi plan deployed, both header + query-param conditions → never trips 429 while non-conditional limits do; flag to product team). CRUD + all non-conditional 429 enforcement already covered. |
